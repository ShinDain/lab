...
GLvoid drawScene() //--- 콜백 함수: 그리기 콜백 함수
{
	...
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(Rlarge * Rself * T1 * T2 * PS2 * PS2 * PS1));
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);

	glUniform3f(objColorLocation, 1.0f, 1.0f, 0.0f);
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(Rlarge * Rself * T1 * PS2 * PS1 * PS1));
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);

	glUniform3f(objColorLocation, 0.0f, 0.0f, 1.0f);
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(Rlarge * Rself * T1 * T2 * T1 * PS2 * PS1));
	glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, 0);
...
void Timer(int value)
{
	for (int i = 0; i < 20; i++)
	{
		if (snow_pos[i][1] == NULL || snow_pos[i][1] < -2.0f)
		{
