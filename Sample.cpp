...
#define CAMERA_POS		glm::vec3(0.0, 0.0, 200.0)
...
GLvoid drawScene()
{
	...
	GLuint modelTransformLocation = glGetUniformLocation(s_program, "Transform");
	...
	GLuint projectLocation = glGetUniformLocation(s_program, "projectionTransform");
	glUniformMatrix4fv(projectLocation, 1, GL_FALSE, glm::value_ptr(proj));

	glm::vec3 lightAmbient = LIGHT_AMBIENT;
	GLuint lightAmbientLocation = glGetUniformLocation(s_program, "lightAmbient");
	glUniform3fv(lightAmbientLocation, 1, (float*)&lightAmbient);

	glm::vec3 lightPos = LIGHT_POS;
	GLuint lightPosLocation = glGetUniformLocation(s_program, "lightPos");
	glUniform3fv(lightPosLocation, 1, (float*)&lightPos);

	glm::vec3 lightColor = LIGHT_COLOR;
	GLuint lightColorLocation = glGetUniformLocation(s_program, "lightColor");
	glUniform3fv(lightColorLocation, 1, (float*)&lightColor);

	glm::vec3 objColor = OBJECT_COLOR;
	GLuint objColorLocation = glGetUniformLocation(s_program, "objectColor");
	glUniform3fv(objColorLocation, 1, (float*)&objColor);

	GLuint cameraPosLocation = glGetUniformLocation(s_program, "cameraPos");
...
void make_vertexShaders()
{
	char* vertexsource = filetobuf("vertex_light2.glsl");
...
void make_fragmentShaders()
{
	char* fragmentsource = filetobuf("fragment_light2.glsl");
